...
image_paths = glob.glob('images\\3_images\\*.jpg')
images = []
...
if not error:
    ...
    dim = (width, height)
    resized_panorama = cv2.resize(
        stitched_img, dim, interpolation=cv2.INTER_AREA)

    cv2.imwrite("stitchedOutput.png", resized_panorama)
    cv2.imshow("Stitched Img", resized_panorama)
    cv2.waitKey(0)
    ...
    cv2.imwrite("stitchedOutputProcessed.png", resized_panorama)

    cv2.imshow("Stitched Image Processed", resized_panorama)
